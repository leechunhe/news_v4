...
function loadXMLAndDisplay() {
  fetch('DSnoPhi.xml') // đường dẫn đến file XML
...
function displayPage(page) {
  ...
  contentDiv.innerHTML = `
	...
	<p>Tìm kiếm nhanh: <input type="text" id="searchBox" placeholder="Nhập Mã DV hoặc Tên DV..." style="padding: 6px; width: 300px;">
  <button onclick="performSearch()" style="padding: 6px 12px; margin-left: 5px;">Tìm kiếm</button></p></br>
    <table border="1" width="100%">
      <tr  style="background-color: #007bff; color: white;"><th>STT</th><th>Mã DV</th><th>Tên DV</th><th>Tổng Tiền</th></tr>
      ${pageData.map(item => `
        <tr>
          <td>${item.STT}</td>
		  <td>${item.MaDV}</td>
...
function createPagination(currentPage = 1) {
  const totalPages = Math.ceil(dataList.length / ROWS_PER_PAGE);
